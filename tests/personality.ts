...
describe("Personality Test", () => {
  ...
  async function getProfile() {
    const [personalityPda, _] = await PublicKey.findProgramAddressSync(
      [Buffer.from("personality"), wallet.publicKey.toBuffer()],
      program.programId
    );
    return await program.account.personality.fetch(personalityPda);
  }
  ...
  it("Create Personality", async () => {
    ...
    const tx = await program.methods
      .createPersonality(name, age, hobbie, message)
      .accounts({
  ...
  it("Update Personality", async () => {
    ...
    const tx = await program.methods
      .updatePersonality(name, age, hobbie, message)
      .accounts({
        signer: wallet.publicKey,
      })
      .signers([wallet])
    ...
    for (const testCase of testCases) {
      ...
      try {
        await program.methods
          .createPersonality(testCase.name, testCase.age, testCase.hobbie, testCase.message)
          .accounts({
